 ...
 *
 * - [ ] Change colors mode (only change attrs, leave chars)
 *
 * - [X] Colors
 * - [ ] Undo
 * - [ ] Load config
...
int main(void) {
	...
	if ( !mousemask(BUTTON1_CLICKED | BUTTON1_PRESSED | BUTTON1_RELEASED |
	                ...
	                NULL) ) {
		fprintf(stderr,
		        "No mouse events can be captured. Try a different terminal "
	...
	///
	/// or simply print this sequence to enable mouse position reporting,
	/// and leave the terminal on xterm-256color
	printf("\033[?1003h");
	fflush(stdout);
	...
	loop {
		...
		case '\n':
			curs_set(CURSOR_VISIBLE);
			write_char(buffer, y, x, current_char, current_color_id,
			           current_attrs);
			break;
		case '\b': /* '^h' */
			curs_set(CURSOR_VISIBLE);
...
fn draw_ui() {
	...
	foreach (color_id, 0, COLORS_LEN) {
		...
		foreach (ltr, 0, COLS / COLORS_LEN) {
			addch(' ');
			/* struct CEntry *e = */
			/*     &buffer[0][color_id * palette_element_width_chars
			 * + ltr]; */
			/* e->ch = 'X'; */
			/* e->color_id = color_id; */
			/* e->attrs = CE_NONE; */
 ...
 */
Result load_from_file(struct CEntry buffer[LINES][COLS], int insert_pos_y,
	...
	if ( header[0] != 'C' || header[1] != 'E' ) {
		log_add(LOG_WARN, "Format not recognized: File header should "
		                  "begin with 'CE'\n");
		return no_input;
	}
	...
	if ( insert_lines > LINES || insert_cols > COLS ) {
		log_add(LOG_WARN,
		        "File %s cannot be loaded (dimensions of saved buffer: "
		        "%d x %d, "
		        "terminal: %d x %d)\n",
	...
	log_add(LOG_INFO, "Loading %dx%d bytes from %s\n", insert_lines,
	        insert_cols, currently_open_file);
	...
	/* insert_partial_buffer(buffer, clip_buf, y, x, insert_lines,
	 * insert_cols);
	 */
